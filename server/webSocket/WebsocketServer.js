...
function startWebSocketServer(sessionParser, server) {
  ...
  wss.on("addGame", (ws, userId, lobbyId, data) => {
    ...
    if (team1Turn) {
      ...
      pickingPlayerIndex =
        Math.floor(totalGames / 2) % lobby.teamTwo.members.length;
  ...
  wss.on("endTeamDraft", (ws, userId, lobbyId) => {
    ...
    if (userId !== lobbyHost) {
      console.log(
        `User ${userId} is not the host of lobby ${lobbyId} cause host is ${lobby.host}`,
      );
      return;
    }
    lobby.isOpen = false;
  ...
  wss.on("endGameDraft", (ws, userId, lobbyId) => {
    console.log(
      "ending game draft, current lobby state: ",
      lobbies.get(lobbyId),
    );
  ...
  wss.on("setGameState", (ws, userId, lobbyId, data) => {
    ...
    if (data.teamOne.score + data.teamTwo.score === data.games.length) {
      if (data.teamOne.score > data.teamTwo.score)
        broadcast(lobbyId, "Team One");
      if (data.teamOne.score < data.teamTwo.score)
        broadcast(lobbyId, "Team Two");
